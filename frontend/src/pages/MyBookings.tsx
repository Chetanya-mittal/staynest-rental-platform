...
const MyBookings = () => {
  ...
  const handleCancelConfirm = async () => {
    ...
    try {
      await cancelBookingApi(cancelTargetId)
      setBookings((prev) =>
        prev.map((b) =>
          b._id === cancelTargetId ? { ...b, status: "cancelled" } : b
        )
      )
    } catch (err: unknown) {
      setError(getErrorMessage(err))
    ...
    <div className="container flex flex-col items-center">
      <div className="mb-6 space-y-1 flex flex-col items-center">
        ...
        <p className="text-sm text-muted-foreground">
          Trips you've booked on StayNest
        </p>
      </div>
        ...
        <Card className="w-full max-w-2xl border-dashed border-border/70 bg-background/50">
          <CardContent className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="mt-2 max-w-md text-sm text-muted-foreground">
              You haven't made any bookings yet. Start exploring stays and plan
              your next trip.
            </p>
      ...
      <ConfirmDialog
        ...
        message="This will free up the dates for other guests. This action cannot be undone."
